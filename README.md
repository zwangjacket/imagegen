...
If you are new to this repository, start with the Quick start.
If you want to add support for additional fal.ai models or workflows, see the Developer guide.
...
```
# clone
git clone <this-repo-url> imagegen
cd imagegen
...
Set your fal.ai API key in the environment. The CLI auto-loads .env,
so the recommended way to handle this is to copy the dotenv-example to .env and edit it.
...
- Example run:
```
uv run imagegen seedream -f cookie_example
```
Note: seedream is an existing model on fal.ai, 
and the above example command will render the image of a cookie into assets/.
...
If you name your files differently, you must provide the extension.

If you place your files elsewhere, you must provide a full pathname.

If you use `-p` instead of `-f`, you can provide the prompt string on the command line.
...
### Output
- Images are downloaded to assets/ with an autogenerated filename.
...
This section targets contributors who want to expose additional fal.ai endpoints in the CLI.
The CLI reflects models from a single registry in code; to add a model or workflow,
extend the registry and follow the option schema described below.
...
- flags: optional list of CLI flags.
  Examples: ["-#", "--num-images"], ["-s", "--seed"].
  If omitted, a default --kebab-case flag is generated.
